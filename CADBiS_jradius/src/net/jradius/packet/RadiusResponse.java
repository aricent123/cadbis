...
public abstract class RadiusResponse extends RadiusPacket
{
     ...
     */
    public boolean verifyAuthenticator(RadiusClient client, byte[] requestAuthenticator)
    {
        byte[] attribtues = RadiusFormat.getInstance().packAttributeList(getAttributes());
        byte[] hash = RadiusUtils.makeRFC2865ResponseAuthenticator(
                client.getMD(), client.getSharedSecret(),
