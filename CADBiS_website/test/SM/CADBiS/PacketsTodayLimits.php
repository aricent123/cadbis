...
class PacketsTodayLimits
{
	...
	protected function recalcTrafficLimits()
	{
		...
		for($i = 0; $i< count($this->packets); ++$i)
			$this->packetsCoefs[$this->packets[$i]['gid']] = $this->packets[$i]['rang'] * $this->packets[$i]['simuluse_sum']/ $SumOfRangs;
			
		for($i = 0; $i< count($this->packets); ++$i)
		{
			$dayLimit = round($allowedDayTraffic * $this->packetsCoefs[$this->packets[$i]['gid']]);
