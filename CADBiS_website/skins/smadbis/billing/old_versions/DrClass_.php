...
function GetUserTotalAcctsData($uid)
{
    ...
    $query="SELECT sum(time_on),sum(in_bytes),sum(out_bytes) from `".$GV["actions_tbl"]."` where user='".$user."';";
...
function GetPeriodTotalAccts($fdate,$tdate)
	{                                  
	global $GV;
       	$query="select sum(out_bytes),sum(in_bytes),sum(time_on) from `".$GV["actions_tbl"]."` where start_time>='".$fdate."' and stop_time<='".$tdate."';";
	$result=mysql_query($query);
	$row=mysql_fetch_array($result);
	$res=NULL;
	$res["traffic"]=(int)$row["sum(out_bytes)"]+(int)$row["sum(in_bytes)"];
...
function GetMonthUsersAccts($order=">traffic",$draw=false)
	{                                  
	global $GV;
        if($order==">user")
          $uorder=" order by `user` desc";
        elseif($order=="<user")
          $uorder=" order by `user` asc";        
        elseif($order==">fio")
          $uorder=" order by `fio` desc";
        elseif($order=="<fio")
          $uorder=" order by `fio` asc";        
        else
          $uorder="";
	$query="select user,fio,uid,nick from `".$GV["users_tbl"]."`".$uorder.";";
	$result=mysql_query($query,$this->link)or die($query." : ".mysql_error());
	if(mysql_num_rows($result)==0){return NULL;}
	$users=NULL;
	$k=0;
...
//Трафик Пользователей за сегодня
function GetTodayUsersAccts($order=">traffic",$draw=false)
	{                                  
	global $GV;           
        if($order==">user")
          $uorder=" order by `user` desc";
...
function GetWeekUsersAccts($order=">traffic",$draw=false)
	{                                  
	global $GV;
        if($order==">user")
          $uorder=" order by `user` desc";
...
function GetSessions($fdate,$tdate)
	...
	global $GV;
       ...
       $res=NULL;
        $query="select * from `".$GV["actions_tbl"]."` where start_time>'".$fdate."' and stop_time<'".$tdate."' order by `start_time`;";
	  $result=mysql_query($query);
          $k=0;
          while($row=mysql_fetch_array($result))
            {      
...
function FindUsers($searchstr,$see)
	{
	global $GV;
	$ok=false;
	$like="";
	...
	$query="SELECT `uid` from `".$GV["users_tbl"]."`".$like.";";
	//die($query);
	$result=mysql_query($query,$this->link)or die("Invalid query(Add User): " . mysql_error());
...
function UpdateUser($uid,$data)
	...
	$query="Update `".$GV["users_tbl"]."` set `password`='".$data[password].
        ...
        `total_traffic`='".$data["total_traffic"]."' where `uid`='".$uid."' ;";
        //die($query);
...
function AddEvent($data)
	...
	$query="Insert into `".$GV["events_tbl"].";";
	$result=mysql_query($query,$this->link)or die("Invalid query(Add Event): " . mysql_error()); 
	return "";
}

//Показать все события
function GetEventsByDate($bdate,$adate)
	{
	global $GV;
        $query="SELECT * from `".$GV["events_tbl"].
        "where `date`>'".$bdate."' and `date`<'".$adate."';";
	$result=mysql_query($query,$this->link)or die("Invalid query(Get Events by Date): " . mysql_error()); 
