...
for($i=1;$i<=$daycount;++$i)          
  ...
  $day=date("D",makeunixtime($year,$month,$i+1,0,0,0));
    ...
    $m=$daycount-$i+1;   
    $daynorm[$i-1]=$rest/$m;
  //echo(($d).")($day) �������: ".bytes2mb($rest)."�� �����: ".bytes2mb($daynorm[$i-1])."�� ���������� �����: ".bytes2mb($accts["traffic"])."�� �� ���� ����:".bytes2mb($test["traffic"])."<br>");
  }
...
?>
<div align=center style="font-size:11px"><b>Статистика использования трафика по дням:</b></div>
...
<td width=50%>
 <table align=center height=180px width=200px >
  ...
  <td class=tbl1>Скачано:<?=make_fsize_str($cur["traffic"]) ?><br> (<? OUT(round($prc_tr,3)) ?>%)</td>
  </tr>
 </table>
</td>
<td width=50% height=100% valign=bottom class=tbl1><?= utils::cp2utf($months[date("n")-1]) ?>(норма <?=make_fsize_str($daynorm[0]) ?>):
