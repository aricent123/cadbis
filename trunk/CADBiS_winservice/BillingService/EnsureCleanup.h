...
Purpose: These classes ensure object cleanup when an object goes out of scope.
         класс гарантирующий уничтожение обэекта(т.е. освобождение памяти) 
...
// класс шаблон
// TYPE тип класса
// PFNENSURECLEANUP pfn адрес функции производящий очитску памяти
// UINT_PTR tInvalid = NULL указатель соответствующий невалидному состоянию объекта
//							по умолчанию - NULL
...
public:
  ...
  //переназначение обекта , удаляем уже охраняемый объект
   ...
   //поддержка 32 и 64 битной версии windows
   //работа с охранямым обектом
   operator TYPE() { 
   ...
   void Cleanup() { 
      if (IsValid()) {
         ...
         pfn(m_t);         // Close the object.
							//вызываем pfn() чтобы с ей помощью очистить объект
         m_t = tInvalid;   // We no longer represent a valid object.
							//теперь хранится невалидный объект
      }
   }
  //в закрытой части сам объект
private:
...
//макросы для простого создания классов для конкретных типов данных
...
//Образцы классов шаблонов для общих типов данных.
...
//специальный класс для освобождения занятой памяти
//необходим тк VirtualFree принимает  3 аргумента
...
//специальный класс для освобождения блока динамической памяти
//необходим тк VirtualFree принимает  3 аргумента
